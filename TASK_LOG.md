...
GL timing, item 6. The annotated gate level result now lives in docs/GL_TIMING.md, not only a log line. max_ss_100C_1v60, pass, 9 of 9, annotation confirmed applied by the missing file probe. The word attempt is retired in test/Makefile.sdf and test/sdf_annotate.v.

W1 composition, item 7, a real sharpening. The 3.4x charged the worst single pin. Paths compose. The worst setup path at max_ss_100C_1v60, _1595_ to _1622_, traverses six waived nets in series, net51 net50 net22 net21 net20 net14, slew excesses 0.114, 0.001, 0.342, 0.190, 0.452, 0.002 ns, summing to about 1.10 ns. Charging every excess in full as added delay, the path still holds plus 0.46 ns, 1.41x inside its 1.5557 ns slack. Tighter than the reviewer's two net estimate of 0.92 ns and 1.7x, and it still clears. The composition sentence is in docs/WAIVERS.md.

Wording sweep. SPEC_NOTES.md rebased to the current frozen spec, the strobe low across reset release rule is in force now and removed from the queue, only the LDB_MAC feature remains. CDC_MTBF.md spoken section corrected 22 to 23 white box tests, the latency grid was uncounted, and the Beer and Ginosar citation gained its venue, The Devolution of Synchronizers, IEEE ASYNC 2010. README wart row names the three ss corners and max tt, not just ss. The phrase two waivers does not appear anywhere in the tree, checked, the docs already frame it as one waiver W1 with two classes, and WAIVERS.md now says so in one line, the nearest surface phrasing was two waived warts on the study page, which is accurate.
