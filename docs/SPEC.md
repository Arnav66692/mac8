...
A serial signed multiply accumulate unit. 8 bit signed times 8 bit signed, accumulated into 24 bit signed with saturation. One multiplier, one accumulator, one command decoder. The atom of AI math, small enough to tape out, deep enough to interview on.

## Signals from the Tiny Tapeout harness

clk, the core clock, 50 MHz nominal on the Tiny Tapeout harness. rst_n, active low reset, asynchronous at the pad. The Tiny Tapeout clock spec states that both the clk and rst_n pins are handled like any other input pins, no harness synchronization, tinytapeout.com/specs/clock. The design synchronizes rst_n through two flops, assertion and release reach the core 2 to 3 clocks after the pad edge, phase dependent, the edge lands anywhere in a clock cycle and the first sampling edge adds 0 to 1 clocks before the 2 flop crossing. ena, high when this design is selected, ignored internally in v0.1. ui_in[7:0], dedicated inputs. uo_out[7:0], dedicated outputs. uio[7:0], bidirectional with direction set statically by the design.
...
Commands arriving while busy is high are ignored. The design ignores further accepts for 3 clocks after any accept, and the minimum legal rise to rise spacing is 5 clocks, so a compliant driver never loses a command, including at the worst async alignment where accepts from two legal rises land 4 clocks apart. The precise ringing guarantee is that lockout window. A re crossing of a ringing or slow strobe edge whose accept lands 1 to 3 clocks after the accepted edge is suppressed, which covers any ring inside the legal pulse shape. A re crossing later than that, below the pulse timing contract, is indistinguishable from a second intended strobe and may execute, use external debouncing for slow edges. Below the legal spacing the lockout is best effort, whether a sub contract rise passes or blocks depends on which edge resolves slow, the requirements table states it. At demo board speeds, an MCU toggling GPIO, these numbers are trivially met.
...
acc 0. Overflow flag 0. Busy 0. Output select LO. uo_out 0. These values hold once the synchronized reset has been sampled and acted on. Outputs are undefined before that, from power up they can read anything until the documented sampled edges have passed.

## Reset rule

The reset is synchronous and needs a running clock. It acts only on rising clock edges, so rst_n held low with the clock stopped resets nothing, and no state clears until clocking resumes. Hold rst_n low at least 3 clocks for the reset to take effect, the pad level crosses the internal synchronizer and acts in 2 to 3 clocks, phase dependent, the pad edge lands anywhere in a clock cycle. Hold strobe low across reset release. The first command after reset requires strobe observed low, then a fresh rise. Hold strobe low at least 5 clocks after release before that first rise. The release crosses the 2 flop synchronizer in 2 to 3 clocks, then the arm settles 3 internal clocks later, the fifth or sixth clock after the pad release, phase dependent. The derived hard floor is 3 clocks, and a rise at release itself is read as strobe high across reset and dropped. Counts before v0.4 were 3 and 1, measured without the reset synchronizer.
